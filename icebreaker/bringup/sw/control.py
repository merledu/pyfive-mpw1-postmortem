...
class MidiInputTest(BaseTest):
	...
	def run(self):
		...
		# Load firmware
		self.build_and_load_fw("demo-midi-input.S")

		# Flush FIFO
		while True:
			w = self.ctrl.wb.read(0x20000)
			if w & (1 << 31):
				break
		...
		# Configure UART
		self.ctrl.wb.write(0x20001, 96-2)

		# Test loop
		v = 0x11
		while True:
			self.ctrl.wb.write(0x20000, v)
			time.sleep(0.1)
			w = self.ctrl.wb.read(0x20000)
			print(f"Wrote {v:02x}, Read {w:02x}")
			v = (v + 0x11) & 0xff
